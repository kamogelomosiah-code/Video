...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <header className="h-16 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-900 flex items-center justify-between px-4 sticky top-0 z-30 flex-shrink-0">
      
      {/* Mobile Search Overlay */}
      {showMobileSearch ? (
         <div className="absolute inset-0 bg-zinc-950 flex items-center px-4 z-40 animate-fade-in">
        ...
        <>
          <div className="flex items-center flex-1">
            ...
            <div className="relative max-w-md w-full ml-4 hidden md:block">
              ...
              <input type="text" placeholder="Search creators, professionals, or content..." className="w-full bg-black border border-zinc-800 rounded-full py-2 pl-10 pr-4 text-sm text-zinc-200 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all" />
            </div>
          </div>
          ...
          <div className="flex items-center space-x-1 sm:space-x-4">
            ...
            {!isGuest && !user.verified && (
              <button type="button" onClick={onVerifyClick} className="hidden md:flex items-center space-x-2 px-3 py-1.5 bg-red-600/10 text-red-500 rounded-full text-xs font-medium hover:bg-red-600/20 border border-red-600/20 transition-all">
            ...
            <div className="relative" ref={notificationsRef}>
                <button type="button" aria-label="Toggle notifications" onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 text-zinc-400 hover:text-red-500 transition-colors">
                    <Bell className="w-5 h-5" />
                    {unreadCount > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-red-600 rounded-full border-2 border-zinc-950 animate-pulse"></span>}
                </button>
                {showNotifications && (
                    <div className="absolute right-0 mt-2 w-80 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl z-50 overflow-hidden">
                        <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-950/50 backdrop-blur-sm"><h3 className="font-bold text-white text-sm">Notifications</h3>{unreadCount > 0 && <button type="button" onClick={handleMarkAllRead} className="text-xs text-red-500 hover:text-red-400 font-medium">Mark all read</button>}</div>
                        <div className="max-h-80 overflow-y-auto">{notifications.length === 0 ? <div className="p-8 text-center text-zinc-500 text-sm">No notifications yet.</div> : notifications.map(notif => (<div key={notif.id} onClick={() => handleNotificationClick(notif.id)} className={`p-4 border-b border-zinc-800/50 hover:bg-zinc-800 cursor-pointer transition-colors ${!notif.read ? 'bg-red-600/5' : ''}`}><div className="flex items-start"><div className={`w-2 h-2 rounded-full mt-2 mr-3 flex-shrink-0 ${!notif.read ? 'bg-red-600' : 'bg-transparent'}`}></div><div><p className={`text-sm ${!notif.read ? 'text-white font-medium' : 'text-zinc-400'}`}>{notif.message}</p><p className="text-xs text-zinc-600 mt-1">{notif.createdAt}</p></div></div></div>))}</div>
                    </div>
                )}
            </div>
            ...
            <div className="flex items-center space-x-3 border-l border-zinc-800 pl-2 sm:pl-4">
              {isGuest ? (
                 <button type="button" onClick={onLoginClick} className="flex items-center space-x-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg shadow-red-600/20 transition-all transform hover:scale-105 active:scale-95"><LogIn className="w-4 h-4" /><span className="hidden sm:inline">Sign In</span></button>
              ) : (
                <div className="relative" ref={userMenuRef}>
                  <button type="button" onClick={() => setShowUserMenu(!showUserMenu)} className="relative group flex items-center space-x-2 focus:outline-none">
                    <img src={user.avatarUrl} alt="User" className="w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-zinc-800 group-hover:border-red-600 object-cover transition-colors"/>
                  ...
                  {showUserMenu && (
                     <div className="absolute right-0 mt-2 w-48 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-50 overflow-hidden animate-fade-in-up">
                        ...
                        <button type="button" onClick={() => { onLogout(); setShowUserMenu(false); }} className="w-full text-left px-4 py-3 text-sm text-red-500 hover:bg-zinc-800 hover:text-red-400 flex items-center transition-colors border-t border-zinc-800"><LogOut className="w-4 h-4 mr-2"/>Sign Out</button>
                     </div>
                  )}
                </div>
              )}
