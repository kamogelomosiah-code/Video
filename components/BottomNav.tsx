...
const BottomNav: React.FC<BottomNavProps> = ({ activePage, onNavigate, onMenuClick }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-zinc-950/90 backdrop-blur-lg border-t border-zinc-800 z-40 md:hidden safe-area-pb">
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`flex flex-col items-center justify-center w-full h-full space-y-1 active:scale-95 transition-transform ${
              activePage === item.id ? 'text-red-500' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
